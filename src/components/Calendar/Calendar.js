  ...
  return (
    <div className="w-full max-w-6xl mx-auto bg-white shadow-lg rounded-lg overflow-hidden">
      ...
      <div className="grid grid-cols-8 bg-gray-100 border-b border-gray-300 text-center font-semibold">
        ...
        {daysOfWeek.map((day) => (
          <div key={day} className="p-2 text-gray-700">
            {day}
          </div>
        ))}
      </div>
        ...
        {daysOfWeek.map((_, dayIndex) => (
          <div key={dayIndex} className="flex flex-col relative">
            {hours.map((_, hourIndex) => (
              <div
                key={hourIndex}
                className="h-14 border-b border-gray-200 hover:bg-gray-100 relative"
              ></div>
            ))}
            {/* Render events */}
            {events
              ...
              .map((event, index) => (
                <div
                  key={index}
                  className={`absolute w-full left-0 bg-${event.color} border-l-4 border-${event.border} p-2 text-sm font-medium shadow-md text-black rounded-none`}
                  style={{
